...
def main(
    ...
    vectorised_simulate = jax.vmap(
        simulate,
        in_axes=(0, None, None), # vectorize over the rule
        out_axes=0,
    )
    ...
    states_rts = compiled_simulate(
        jnp.arange(256), # simulate all 256 rules in parallel
        width,
        num_steps,
    ).block_until_ready()
    end_time = time.perf_counter()
    ...
    end_time = time.perf_counter()
    print("simulation complete!")
    print(f"time taken {end_time - start_time:.5f} seconds (third execution)")
...
def simulate(
    ...
    initial_state = initial_state.at[width//2].set(1) # Was originally: state[width//2] = 1

    # simulate (with jax.lax.scan)
